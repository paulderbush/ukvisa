...
UK_MAP_JS = r"""
const UK_NATIONS = {
  scotland: { name: 'Шотландия', cities: ['Эдинбург', 'Глазго', 'Инвернесс'] },
  england:  { name: 'Англия',    cities: ['Лондон', 'Манчестер', 'Бирмингем', 'Ливерпуль'] },
  wales:    { name: 'Уэльс',     cities: ['Кардифф', 'Суонси'] },
  nireland: { name: 'Сев. Ирландия', cities: ['Белфаст'] },
};
    ...
    <section id="map" style={{ paddingBlock: 'var(--section-gap)' }}>
      <div className="rv-container">
        <div style={{ textAlign: 'center', maxWidth: 680, margin: '0 auto 44px' }}>
          <span className="rv-eyebrow">Великобритания</span>
          <h2 style={{ fontSize: 'var(--t-h1)', marginTop: 14 }}>Одна виза — вся страна</h2>
          <p style={{ marginTop: 16, fontSize: 'var(--t-lg)', color: 'var(--text-body)' }}>
            Британская виза открывает доступ ко всем четырём нациям — Англии, Шотландии, Уэльсу и Северной Ирландии.
          </p>
        </div>
        <div style={{ maxWidth: 700, margin: '0 auto' }}>
          <div style={{ padding: 12, borderRadius: 'var(--r-2xl)', background: 'var(--glass-fill)', border: '1px solid var(--glass-edge)', backdropFilter: 'var(--glass-blur)', WebkitBackdropFilter: 'var(--glass-blur)', boxShadow: 'var(--elev-2), var(--glass-inner)' }}>
            <img src="assets/photos/ukmap.png" alt="Карта Великобритании" style={{ display: 'block', width: '100%', height: 'auto', borderRadius: 20 }} />
          </div>
          <div style={{ marginTop: 24, display: 'flex', flexWrap: 'wrap', gap: 12, justifyContent: 'center' }}>
            {Object.entries(UK_NATIONS).map(([key, nation]) => {
              const on = hovered === key;
              return (
                <div key={key} onMouseEnter={() => setHovered(key)} onMouseLeave={() => setHovered(null)} style={{
                  padding: '14px 20px', borderRadius: 'var(--r-lg)', cursor: 'default', minWidth: 160,
                  ...
                  <div style={{ fontWeight: 600, color: on ? 'var(--text-strong)' : 'var(--text-body)', fontSize: 'var(--t-body)' }}>{nation.name}</div>
                  <div style={{ fontSize: 'var(--t-sm)', color: 'var(--text-muted)', marginTop: 4 }}>{nation.cities.join(', ')}</div>
                </div>
              );
            })}
          </div>
